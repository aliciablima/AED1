...
Soldado eliminarJosephus(Deque *D, int n, int soldadoInicial) {
    Soldado soldadoEliminado;
    int contador = 1;
    
    while (D->inicio->soldado.codigo != soldadoInicial) {
        Soldado temp = removerInicio(D);
        inserirFim(D, temp);
    }
